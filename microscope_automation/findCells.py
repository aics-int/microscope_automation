...
# from . import samples
from . import automationMessagesFormLayout as form
...
class CellFinder:
    ...
    def segment_image(self):
        ...
        :param image: ImageAICS type image to be segmented
        :return seg_image: segmented image
        """
    ...
    def export_cells(self):
        ...
        for (ind, point) in enumerate(self.image_locations, start=1):
            ...
            cell_to_add = Cell(name=name, center=[x_pos, y_pos, 0], colonyObject=self.colony_object)
    ...
    def get_original_data(self):
        ...
        :param image: ImageAICS image
        :return: 2D numpy array of image data
        """
        ...
        if self.seg is None:
            ...
            if len(img_data.shape) == 2:
                ...
                # self.original_data = self.original.get_data()[:, :, self.prefs.getPref('ImageIndex')].astype(np.float)
                # self.original_data = np.transpose(self.original_data, (0, 1))
                self.original_data = self.original.get_data()[:, :, self.prefs.getPref('ImageIndex')].astype(numpy.float)
            # self.original_data = np.transpose(self.original_data, (1, 0))
            # self.original_data /= self.original_data.max()
            self.original_data = exposure.rescale_intensity(self.original_data, out_range=(0., 1.))
    ...
    def adjust_image(self):
        ...
        :param image: 2D numpy array of image data
        :return: 2D numpy array of image data
        """
        ...
        if self.calibrate:
            ...
            if self.validate("Calibrate Contrast", "Calibrate sigmoid and gamma adjustment values",
                             [fields, names, values]):
    ...
    def initial_edge_detection(self):
        ...
        if self.calibrate:
            ...
            if self.validate("Calibrate Initial Edge Detection",
    ...
    def improve_initial_edges(self):
        ...
        if self.calibrate:
            ...
            if self.validate("Calibrate Dilation and Hole Filling",
    ...
    def filter_objects(self):
        ...
        :param image: 2D numpy array of image data
        :param prefs: preferences for segmentation
        :return: 2D numpy array of image data
        """
    ...
    def twofold_distance_map(self):
        ...
        colony_only = ndimage.gaussian_filter(self.original_data, values[2]) # might need to switch this from original to adjusted image
        ...
        # helper function to avoid repeated code, checks with user that point chosen is suitable
        def verify_smooth_point():
            smooth_point = numpy.where(inner_edges == inner_edges.max())
            point = numpy.copy(self.original_data)
            point[smooth_point[0][0]-3:smooth_point[0][0]+3, smooth_point[1][0]-3:smooth_point[1][0]+3] = 1
            self.viewer.display_image(point)
            if self.validate("Chosen point", "Is this chosen point acceptable?", [], default=True):
                self.viewer.close()
        ...
        if self.calibrate:
            ...
            if self.validate("Calibrate Twofold Distance Detection",
    ...
    def validate(self, form_title, form_comment, pref_data, default=False):
        ...
        :param form_title: title of form to be displayed
        :param form_comment: comment for form to be displayed
        :param pref_data: (list) contains form fields, preference names, preference values, and (in that order)
        :param default: checked or unchecked as default for continuing
        :return: boolean for whether to continue recursion
        """
        assert len(set(len(x) for x in pref_data)) <= 1, "Mismatched parameter lengths"
        args = [(pref_data[0][i], pref_data[2][i]) for i in range(len(pref_data[2]))] if len(pref_data) else []
        ...
        elif not result[-1] and not all(result[i] == pref_data[2][i] for i in range(len(pref_data[2]))):
...
class CalibrationViewer:
    ...
    def close(self):
        ...
        for proc in psutil.process_iter():
            if proc.name().startswith("display") or proc.name().startswith("PicasaPhotoViewer"):
                proc.kill()


def display_and_save(image, name, location="/home/mattb/git/microscopeautomation/data/test_data_matthew/segmentationtest/", display=False):
    if display:
...
def segment_dir(directory, multi=True):
    file_match = [img for img in filter(lambda x: x.endswith('.tif'),  [os.path.join(directory, loc) for loc in os.listdir(directory)])]
...
def segment(image, img_dir):
    ...
    if len(img_data.shape) == 3:
        ...
        img_data = measure.block_reduce(img_data, (1, 1, 10, 10), func=numpy.mean)[0, 0, :, :]
    ...
    inner_edges = ndimage.distance_transform_edt(~colony_edges * top_percent.astype(bool))
    ...
    smooth_dist[smooth_point[0]-3:smooth_point[0]+3, smooth_point[1]-3:smooth_point[1]+3] = smooth_dist.max() * 3
    display_and_save(smooth_dist, "18SmoothMax", img_dir, False)

    overlay[smooth_point[0]-3:smooth_point[0]+3, smooth_point[1]-3:smooth_point[1]+3] = 0
    ...
    outer_edges = ndimage.distance_transform_edt(~colony_edges * bottom_percent.astype(bool))
    display_and_save(outer_edges, "25OuterDistance", img_dir, False)

    bottom_mask = (bottom_percent * (distance * (distance > (d_max * 0.1)))).astype(bool)
    ...
    rough_dist[rough_point[0]-3:rough_point[0]+3, rough_point[1]-3:rough_point[1]+3] = rough_dist.max() * 3
    display_and_save(rough_dist, "27RoughMin", img_dir, False)

    overlay[rough_point[0]-3:rough_point[0]+3, rough_point[1]-3:rough_point[1]+3] = 1
...
if __name__ == "__main__":
    start = time.time()
    segment_dir('/home/mattb/git/microscopeautomation/data/test_data_matthew/colony_seg_test/', False)
