...
class CreateTilePositions(object):
    ...
    def __init__(self, tile_type = 'noTiling', tile_number = (2, 2), tile_size = (1, 1), degrees = 0):
        '''Create position lists for tiling and multi-position imaging.
    ...
    def create_ellipse(self, nx, ny):
        ...
        ellipse_list = [(x, y, z) for (x, y, z) in rect_list if x**2/(nx/2)**2 + y**2/(ny/2)**2 <= 1]        
        return ellipse_list
    ...
    def rotate_pos_list(self, pos_list, degrees):
        ...
        # rotate
        rotate_list = [(math.cos(rad) * px - math.sin(rad) * py, math.sin(rad) * px + math.cos(rad) * py, pz)
    ...
    def get_pos_list(self, center = (0, 0, 0)):
        ...
        # add offset
        pos_list = [(x + center[0], y + center[1], z + center[2])for (x, y, z) in pos_list]
        return pos_list
    ...
    for tile_type in VALID_TILE_TYPE:
        ...
        for tile_number in [(10, 10), (20, 20), (3, 30), (10, 4)]:
            for degrees in [0, 45, 90, -45]:
                ...
                print ('Tile list for tile type {} with no center and {} tiles: {}'.format(tileList.get_tile_type(),
                                                                                           tileList.get_tile_number(), 
                                                                                           tileList.get_pos_list()))      
                tileList.show()
                center = (5, 10, 15)
                print ('Tile list for tile type {} with center {} and {} tiles: {}'.format(tileList.get_tile_type(), 
                                                                                         center, 
                                                                                         tileList.get_tile_number(),
