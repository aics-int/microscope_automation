...
def get_colony_data(prefs, colony_file):
    ...
    '''
    # setup logging
    logger = logging.getLogger('microscopeAutomation.setupAutomation.get_colony_data')  # noqa
    ...
    colonies.loc[:, 'Center_Y^2'] = colonies.loc[:, 'Center_Y'].apply(
        math.pow, args=(2,))
    colonies.loc[:, 'CenterDistance'] = colonies.loc[:, 'Center_X^2'].add(
        colonies.loc[:, 'Center_Y^2']).apply(math.sqrt)
    count_per_well = pandas.DataFrame(colonies.groupby(['PlateID', 'Well']).size(),
                                      columns=['CountPerWell']).reset_index()
    colonies = pandas.merge(colonies, count_per_well, left_on=['PlateID', 'Well'],
...
def add_colonies(well_object, colonies, hardware_settings, prefs=None):
    ...
    for colony in colonies[wellData].itertuples():
        ...
        colony_object = samples.Colony(name=colony_name, image=True, center=center,
                                       ellipse=ellipse,
...
def add_barcode(name, well_object, layout, prefs=None):
    ...
    center = [float(layout.get_pref('xBarcodePos')),
...
def setup_plate(prefs, colony_file=None, microscope_object=None, barcode=None):
    ...
    if colony_file is not None:
        ...
        colonies = filter_colonies(add_colonies_preferences, colonies,
                                   wellDict=wells_definitions)
    ...
    plate_holder_object = samples.PlateHolder(
        ...
        center=[plate_holder.get_pref('xCenter'),
    ...
    plate_object = samples.Plate(name=plate.get_pref('Name'),
                                 plate_holder_object=plate_holder_object,
                                 center=[plate.get_pref('xCenter'),
    ...
    if barcode is None:
        barcode = message.read_string('Barcode', 'Barcode:', default='123',
                                      returnCode=False)
    ...
    for colIndex in range(ncol - 1):
        ...
        for rowIndex in range(nrow - 1):
            ...
            well_object = samples.Well(name=name,
    ...
    if mean_diameter is not None:
        [well_obj.set_set_diameter(mean_diameter) for well_name, well_obj
         in plate_object.get_wells().iteritems()]
    ...
    if colony_file is not None:
        wellsColoniesList = [add_colonies(well_obj, colonies, specifications,
                                          add_colonies_preferences) for
                             well_name, well_obj
                             in plate_object.get_wells().iteritems()]
...
def setup_slide(prefs, microscope_object=None):
    ...
    center = [float(hardware_settings.get_pref('xCenterPlateHolder')),
    ...
    center = [float(hardware_settings.get_pref('xCenterSlide')),
