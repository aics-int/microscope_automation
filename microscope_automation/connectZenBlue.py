...
from automationExceptions import AutomationError, HardwareError, AutofocusError, AutofocusObjectiveChangedError, \
...
# Create Logger
log = logging.getLogger('microscopeAutomation connectZenBlue')
    ...
    def execute_experiment(self, experiment = None, pos_list = None):
        ...
        # call ZEN API to set experiment
            if experiment is None:
                ...
                expClass = self.Zen.Acquisition.Experiments.GetByName(experiment)
                if pos_list:
                    expClass.ClearTileRegionsAndPositions(0)
                    for pos in pos_list:
                        expClass.AddSinglePosition(0, pos[0], pos[1], pos[2])
        except:
...
##################################################################################################################
    ...
    def z_stack_range(self, experiment_path=None, experiment_name=None):
        ...
        :param experiment_path: path of the experiment file
        :param experiment_name: name of the experiment
        :return: zstack_range: range of the z-stack
        """
        zen_experiment = ZenExperiment(experiment_path, experiment_name)
...
#     test=['execute_experiment', 'test_definite_focus', 'test_definite_focus_interactive', 'snap_image', 'live_mode', 'get_all_objectives', 'get_objective_information', 'trigger_pump', 'test_focus', 'save_image', 'test_stage', 'test_macro']
    test=['test_macro']
