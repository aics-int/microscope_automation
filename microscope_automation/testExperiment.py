...
def test_FocusSetup(experiment_object, verbose = True):
    ...
    if not experiment_object.validate_experiment():
        is_valid = False
        if verbose:
            print('test_FocusSetup: Experiment {} does not exist'.format(experiment_object.experiment_path))
    ...
    if not focus_settings:
        is_valid = False
        if verbose:
            print('test_FocusSetup: Experiment {} has no FocusSetup'.format(experiment_object.experiment_path))
    ...
    for settings in focus_settings:
        if not current_z_set(settings):
            is_valid = False
            if verbose:
                print('test_FocusSetup: In experiment {} Focus Strategy of at least one z-stack is not set to CurrentZ'.format(experiment_object.experiment_path))

    return is_valid

if __name__ == '__main__':
    experiment_path = 'D:\Users\winfriedw\Documents\Carl Zeiss\ZEN\Documents\Experiment Setups\ScanWell_100x.czexp'
    experiment = 'ScanWell_100x.czexp'
    verbose = True
    import hardware
    experiment_object = hardware.Experiment(experiment_path, experiment)
