...
class PositionWriter(object):
    ...
    def __init__(self, zsd, plate, production_path, testing_mode=False):
        ...
        if not os.path.exists(self.path):
            raise OSError("Please check zsd, plate, and production path info given to PositionWriter")
        self.zsd = zsd
    ...
    def write(self, converted=[], dummy='', name_czsh='positions_output.czsh'):
        ...
        to_write = os.path.join(self.path, ".".join(split))
    ...
    def get_next_pos_name(self, test_mode=False, test_file_names=[]):
        ...
        if test_mode:
            ...
            prefixed = [filename for filename in os.listdir(self.path) if
                        filename.startswith("positions_output_")]
