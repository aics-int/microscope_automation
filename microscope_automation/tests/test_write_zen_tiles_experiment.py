...
def test_init():
    ...
    assert testclass.path == "data" + os.path.sep + "1234" + os.path.sep \
                             + "testZsd"
...
def test_convert_to_stage_coords():
    ...
    assert converted_list[0] == {"name": "name0", "actual_x": 1.1,
                                 "actual_y": 1.1, "actual_z": 0.1}
    assert converted_list[1] == {"name": "name1", "actual_x": 101.0,
                                 "actual_y": 101.0, "actual_z": 100.0}
    assert converted_list[2] == {"name": "name2", "actual_x": 201.5,
                                 "actual_y": 201.5, "actual_z": 200.5}
...
def test_get_next_pos_name():
    testclass = write_zen.PositionWriter(zsd="testZsd", plate=1234,
                                         production_path="data")
    # Does file get next letter correctly?
    assert (testclass.get_next_pos_name(test_mode=True,
                                        test_file_names=['positions_output_a']) == 'b')

    # Not backfilling files?
    testclass = write_zen.PositionWriter(zsd="testZsd", plate=1234,
                                         production_path="data")
    assert (testclass.get_next_pos_name(test_mode=True,
                                        test_file_names=['positions_output_aa'])
            == 'ab')
    ...
    assert (testclass.get_next_pos_name(test_mode=True,
                                        test_file_names=['positions_output_a',
                                                         'positions_output_c',
                                                         'positions_output_z'])
            == 'aa')
