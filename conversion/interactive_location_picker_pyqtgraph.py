...
class ImageLocationPicker(object):
    ...
    def __init__(self, image=None, location_list=[], app=None,
        ...
        Input:
         ...
         location_list: List of tuples in the form of (x,y) coordinates of the points selected
         app: QtGui application object initialized in the beginning to automation software in microscopeAutomation.py
        ...
        self.counter = 0
        # Double curly braces ({{}}) in the help string allows for formatting twice, after the single curly braces.
        self.help_string = "Press {} to add points, {} to delete points, and {} to delete all points." \
                           "\nPress {} to toggle failed acquisition tag." \
                           "\n# of Locations Selected = {{}}"\
                           .format(KEY_ADD_POINTS, KEY_DELETE_POINTS, KEY_DELETE_ALL_POINTS, KEY_FAIL_ACQUISITION)
        self.help_text = pyqtgraph.TextItem(self.help_string.format(self.counter), color='g')
        self.failed_text = pyqtgraph.TextItem("This image has been failed", color='r', anchor=(-4, 0))
    ...
    def flip_coordinates(self, location_list):
        '''ZEN has the image origin in the upper left corner, QTgraph in the lower left corner.
    ...
    def plot_points(self, window_title):
        """Plots a given list of location on the image, gives the user ability to edit them
        using add and delete modes
        ...
        # Resizing the window with an aspect ratio to support rectangular images throws off the coordinates
        # that the automation software gives to the user, so this is hard coded to 1100x1100.
        ...
        # Function to call to update the help text on the ui to current self.counter value
        ...
        self.scatterplot = pyqtgraph.ScatterPlotItem(size=self.spot_size, brush=self.spot_brush,
                                                     symbol=self.spot_symbol, pen=self.spot_pen)
        ...
        # Given user selected points, delete from current list of points and decrement counter.
        def _handle_delete_points(plot, points):
            location_selected = self.round_locations(points[0].pos()[0],points[0].pos()[1])
        ...
        def _handle_plot_point(event):
            ...
            if self.mode == ADD_MODE:
                ...
                if location_selected not in self.location_list:
                    ...
                    self.scatterplot.setData(pos=self.location_list)
                    point_index = pyqtgraph.TextItem(str(len(self.location_list)), color='b')
                    point_index.setPos(location_selected[0] + 10, location_selected[1] + 10)
                    w.addItem(point_index)
                    self.text_items.append(point_index)
                    self.counter = len(self.location_list)
                    _display_help_text()
        ...
        # This function works well for undoing the last "fail" but does not remember beyond that
        def _handle_undo():
    ...
    def round_locations(self, pos_x, pos_y):
        ...
        # Reason - when deleting the points, we need to find if the point selected is in the location list
        # The comparison fails if they have different number of digits after the decimal points
...
#
###############################################################################################
def test_ImageLocationPicker(prefs, image_save_path, app):
    ...
    from . import microscopeAutomation
    
    microscope_object = setup_samples.setup_microscope(prefs)
    
    # setup plateholder with slide
    plate_holder_object = setup_samples.setup_slide(prefs, microscopeObject=microscope_object)
    ...
    images = slide_object.acquire_images('Setup_20x', 
                                         'Camera1 (Back)',
...
#     image_save_path = '/Users/winfriedw/Documents/Programming/ResultTestImages'
    ...
    app = QtGui.QApplication([])
    test_ImageLocationPicker(prefs=prefsrefs,
