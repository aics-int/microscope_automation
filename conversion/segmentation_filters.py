...
def filter_by_size(input_image, filter_values):
    ...
    :param input_image: The image (numpy array) to the filter to be applied on
    :param filter_values: list [minimum area, maximum area (optional)]
    :return: filtered_image: Image (numpy array) after applying the filter mask
    """
    # Error Checking - there should be at least one value in the list - for the min area
    min_area = 0
    if not filter_values:
        raise ValueError('Filter Values not provided. Please check the preference file.')
...
def filter_by_distance(input_image, filter_values):
    ...
    :param input_image: The image (numpy array) to the filter to be applied on
    :param filter_values: list, in this case [max distance from the center, number of objects requested]
    :return: filtered_image: Image (numpy array) after applying the filter mask
    """
    ...
    label_objects, nb_labels = ndimage.label(input_image)
    center_objects = ndimage.measurements.center_of_mass(input_image, label_objects, list(range(1, nb_labels + 1)))
...
FILTER_MAPPING = {
    "minArea": filter_by_size, "distFromCenter": filter_by_distance}
