'''
Functions to test module hardware_control.py on spinning disk microscope ZSD_3
...
def set_objective_offset(sample_object, test_settings):
    ...
    for experiment in experiments_list:        
        ...
        microscope_object.initialize_hardware(initialize_components_ordered_dict = {objective_changer_id: {'no_find_surface'}}, 
                                     reference_object = sample_object.get_reference_object(), 
                                     trials = 3,
                                     verbose = test_settings['verbose'])       
    return True
...
def test_definite_focus(sample_object, test_settings, experiment = 'experiment_standard'):  
    ...
    microscope_object.live_mode(camera_id = test_settings['camera_id'], 
                                experiment = test_settings['experiment_standard'], 
                                live = True)
    input('Please focus. Hit Enter when done.')
    focus_drive_id = test_settings['focus_drive_id']
    ...
    for experiment_label in test_settings['magnification_experiments']:
        ...
        for pos_number in range(2):
            microscope_object.live_mode(camera_id = test_settings['camera_id'], 
                                        ...
                                        live = True)
            input('Please move to position {}'.format(pos_number))
        ...
        for pos_number, pos in enumerate(pos_list):
            for experiment_label in test_settings['magnification_experiments']:
                ...
                images = sample_object.acquire_images(experiment = test_settings[experiment_label], 
                                                     ...
                                                     metaDict = None, 
                                                     verbose = test_settings['verbose'])
                input('Was position {} with coordinates {} imaged correct with experiment {}?'.format(pos_number, pos, experiment_label))
    return success
...
def test_set_zero(sample_object, test_settings, image_settings):  
    ...
    microscope_object.live_mode(camera_id = test_settings['camera_id'], 
                                experiment = test_settings['experiment_10x'], 
                                live = True)
    input('Please move to center of tile scan on correct focus position')
    ...
    samples_list = create_output_objects_from_parent_object(find_type = 'Interactive',
    ...
    for experiment_type in test_settings['magnification_experiments']:
        ...
        # set zero position for slide
        input('Please focus on slide surface')
        print('Ok')
        ...
        for cell_object in samples_list[0]:
            posList = cell_object.get_tile_positions_list(image_settings, 
                                                            tile_type = 'NoTiling', 
                                                            verbose = test_settings['verbose'])
            print(('Imaging postion in object coordinates: {}'.format(cell_object.get_zero())))
            ...
            images = cell_object.acquire_images(experiment = experiment, 
                                                 ...
                                                 metaDict = None, 
                                                 verbose = test_settings['verbose'])
            print(('Focus position after imaging: {}'.format(microscope_object.get_information(components_list = ['MotorizedFocus']))))
...
def test_hardware(test, prefs, test_settings):
    ...
    print(('{}'.format(microscope_object.microscope_components_ordered_dict)))
...
#     test_list=['test_hardware', 'set_objective_offset', 'test_microscope', 'test_move_sample', 'test_definite_focus', 'test_parfocality', 'test_set_zero']
    ...
    test_settings = {'camera_id': 'Camera1 (back)',
                     ...
                     'set_offset_experiments': ['Setup_20x', 'Setup_10x'],
#                      'magnification_experiments': ['experiment_20x', 'experiment_10x', 'experiment_100x'],
                     'magnification_experiments': ['experiment_10x', 'experiment_100x'],
