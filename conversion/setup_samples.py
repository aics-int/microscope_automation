...
def get_colony_data(prefs, colonyFile):
    ...
    colonies = coloniesAll.loc[plateSelector, list(colonyColumns.keys())]
...
def filter_colonies(prefs, colonies, wellDict):
    ...
    for well, numberImages in wellDict.items():
...
def setup_plate(prefs, colony_file=None, microscope_object=None, barcode=None):
    ...
    # pumpObject = microscope_object.get_microscope_object(pumpName)
    ...
    if mean_diameter is not None:
        [well_obj.set_setDiameter(mean_diameter) for well_name, well_obj in plate_object.get_wells().items()]
    ...
    if colony_file is not None:
        wellsColoniesList = [add_colonies(well_obj, colonies, specifications, add_colonies_preferences) for
                             well_name, well_obj in plate_object.get_wells().items()]
