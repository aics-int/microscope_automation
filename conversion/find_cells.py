...
class CellFinder:
    ...
    def export_cells(self):
        ...
        for key, value in self.prefs.prefs.items():
...
def segment_dir(directory, multi=True):
    file_match = [img for img in [x for x in [os.path.join(directory, loc) for loc in os.listdir(directory)] if x.endswith('.tif')]]
