...
#################################################################
    ...
    def add_to_image_dir(self, listName, sampleObject=None, position=None):
        ...
        Output:
         none
        '''
        if listName not in list(self.image_dirs.keys()):
    ...
    def get_from_image_dir(self, listName):
        ...
        '''
        sampleObjects = None
        for key in list(self.image_dirs.keys()):
            if listName in list(self.image_dirs.keys()):
    ...
    def set_correction(self, x_correction=1, y_correction=1, z_correction=1,
                       z_correction_x_slope=0, z_correction_y_slope=0,
                       z_correction_z_slope=0, z_correction_offset=0):
...
#         self.z_correction_offset=self.z_correction_offset*z_correction_offset
    ...
    def get_correction(self):
        ...
        Output:
         x_correction, y_correction, z_correction: Correction terms
        '''
        return {'x_correction': self.x_correction,
...
###################################################################################################################
    ...
    def microscope_is_ready(self, experiment, reference_object=None, load=True,
                            ...
                            make_ready=True, trials=3, verbose=True):
    ...
    def move_to_abs_position(self, x=None, y=None, z=None,
                             reference_object=None, load=True, verbose=True):
...
class PlateHolder(ImagingSystem):
    ...
    def get_corrected_stage_position(self, verbose=False):
        ...
        if 'focality_drift_corrected' in list(z_positions.keys()):
...
#         z = self.recover_hardware(self.focusDrive.find_surface, communication_object)
    ...
    def execute_experiment(self, experiment,
        ...
        for key, position in list(information_dict[self.stage_id].items()):
            if len(position) == 0:
                position = [None, None]
...
#                             with focus positions in um
        ...
        for key, position in list(stage_z_corrected.items()):
        ...
        #
        if self.objective_changer_id in list(information_dict.keys()):
...
class Well(ImagingSystem):
    ...
    def get_samples(self, sampleType={'Sample', 'Barcode','Colony'}):
        ...
        samples = {}
        try:
            for name, sampleObject in self.samples.items():
...
class Sample(ImagingSystem):
    ...
    def __init__(self, name='Sample', well_object=None, center=[0, 0, 0],
                 experiment=None,
                 x_flip=1, y_flip=-1, z_flip=1,
                 x_correction=0, y_correction=0, z_correction=0,
                 z_correction_x_slope=0, z_correction_y_slope=0):
...
#         self.xCenter, self.yCenter=center
    ...
    def add_cells(self, cellObjectsDict):
        ...
        for cell in cellObjectsDict.values():
...
#     ph.add_immersionDelivery(immersionDeliverySystemsDict={'Water Immersion': im})
    ...
    c1d5.set_clone('123')
    c2d5 = Colony(name='C2D5', image=True, center=[50, 50, 50],
                  ellipse=[1000, 1000, 1000], meta=meta, well_object=d5,
                  x_flip=1, y_flip=-1, z_flip=1,
                  x_correction=1, y_correction=1, z_correction=1,
                  z_correction_x_slope=0, z_correction_y_slope=0)
...
#         # Find center of well based on 10x objective image
    ...
    if 'test_auto_focus' in test:
        ...
        # get first plate
        plate = list(ph.get_plates().values())[0]
    ...
    if 'image_cells' in test:
        ...
        for wellName, well_object in p.wells.items():
            print('Well: ', wellName)
            ...
            for colName, colObject in well_object.get_colonies().items():
                print('.Colony: ', colName)
                print('Zero colony position: ', colObject.get_zero())
                print(colObject.move_to_zero())
                message.operate_message("Check if position is correct")
                for cellName, cellObject in colObject.cells.items():
